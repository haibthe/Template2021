...
fun ComponentActivity.toggleKeyboard() {
  ...
  imm.toggleSoftInputFromWindow(
    findViewById<ViewGroup>(Window.ID_ANDROID_CONTENT).applicationWindowToken,
...
fun ComponentActivity.isKeyboardOpen(visibleThreshold: Float = 100f): Boolean {
  ...
  return root.rootView.height - measureRect.bottom > ceil(
    (visibleThreshold * Resources.getSystem().displayMetrics.density)
  )
}
...
fun ViewGroup.isKeyboardOpen(visibleThreshold: Float = 100f): Boolean {
  ...
  return rootView.height - measureRect.bottom > ceil(
    (visibleThreshold * Resources.getSystem().displayMetrics.density)
  )
}
